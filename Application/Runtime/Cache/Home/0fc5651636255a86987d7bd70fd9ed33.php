	...
	<head>
		...
		$(function(){
			...
			$('#dg').datagrid({
			    striped:true,
			    method: "GET",
			    url:'<?php echo ($BASEPATH); ?>index.php/Home/Class/loadClassByPage?pageNo=1&pageSize=10',
			    pagination:true,
			    rownumbers:true,
			    frozenColumns:[[
		...
		function refreshData(pageNumber,pageSize){
			$("#dg").datagrid('loading');
			$.getJSON("<?php echo ($BASEPATH); ?>index.php/Home/Class/loadClassByPage?pageNo="+pageNumber+"&pageSize="+pageSize,{},function(result){
		...
		function searchClass(){
			$.post("<?php echo ($BASEPATH); ?>index.php/Home/Class/loadClassByPage",{
				'pageNo'	 :1,
				'pageSize'	 :10,
		...
		*/
		function combineClass(){
			...
			$.post("<?php echo ($BASEPATH); ?>index.php/Home/Class/checkExamToday",{'cids':cids.join(",")},function(data){
				if(data == "ok"){
					...
					$('#combinedHeaderid').combobox({    
					    url:'<?php echo ($BASEPATH); ?>index.php/Home/User/loadAllHeader',    
					    valueField:'uid',    
					    textField:'truename',
					    value:'-1'
					});
					...
					$('#combinedManagerid').combobox({    
					    url:'<?php echo ($BASEPATH); ?>index.php/Home/User/loadAllManager',    
					    valueField:'uid',    
					    textField:'truename',
					    value:'-1'  
					});
		...
		function hebingClasses(){
			...
			$.post("<?php echo ($BASEPATH); ?>index.php/Home/Class/hebingClasses",{
				"cids"			   :cids.join(","),
